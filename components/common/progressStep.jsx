...
const ProgressStep = ({ onSelect, step, selected, isLast }) => {
  const completed = () => {
    return parseInt(step.id) < parseInt(selected.id)
  }
  const current = () => {
    return parseInt(step.id) === parseInt(selected.id)
  }
        ...
        <span className="px-6 py-4 flex items-center text-sm font-medium">
          <span className={getIdContainerClasses()}>
            {completed() ? (
              ...
              <span className={getIdClasses()}>{step.id}</span>
            )}
          </span>
          <span className={getNameClasses()}>{step.name}</span>
        </span>
      </button>
      {isLast && (
        <div
